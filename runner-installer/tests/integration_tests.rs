...
async fn test_unknown_feature() {
    ...
    let stdout = String::from_utf8_lossy(&output.stdout);
    assert!(stderr.contains("Unknown feature: unknown-feature") || stdout.contains("Unknown feature: unknown-feature"));
}
...
async fn test_valid_feature_names() {
    ...
    let output = Command::new("cargo")
        .args(&["run", "--", "--features=nodejs", "--config=/nonexistent/config.yml"])
        .output()
        .expect("Failed to execute command");
