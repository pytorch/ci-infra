...
/*
 * The ArgoCD provider configuration requires some set up
 * outside of Terraform. A token is required, which cannot be
 * generated by terraform.
 * For the very first run we used the local admin account, which
 * is then disabled for security reasons.
 * The service account and its role are defined in the argocd
 * config map deployed via terraform/helm.
 *
 * To create the service token:
 * - temporarly enable local admin, login via CLI as admin
 *    argocd login argocd.pytorch.org
 * - create a token and store in AWS Service Manager:
 *    argocd account generate-token --account terraform-sa
 */

data "aws_secretsmanager_secret_version" "argocd_terraform_sa_token" {
  secret_id = "pytorch-argocd-terraform-sa-token"
}

locals {
  argocd_secret_content     = jsondecode(data.aws_secretsmanager_secret_version.argocd_terraform_sa_token.secret_string)
  argocd_terraform_sa_token = local.argocd_secret_content["token"]
}

provider "argocd" {
  server_addr = local.argocd_ready ? local.argocd_endpoint : null
  auth_token    = local.argocd_ready ? local.argocd_terraform_sa_token : null
}
