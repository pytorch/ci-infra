...
**Architecture**:
- **Central pod** — A StatefulSet with per-pod EBS PVCs clones repositories from GitHub. Uses dual-slot rotation (cache-a / cache-b) so DaemonSet clients always read from a consistent snapshot. Serves repos via rsyncd on port 873. Replica count and resources are configurable per cluster via `clusters.yaml` (`git_cache.*`).
- **DaemonSet** — Runs on every runner/BuildKit node. Discovers central replicas via headless service DNS, queries each replica's metrics to find the one with fewest active rsync connections (least-loaded selection), and rsyncs from it to local NVMe storage at `/opt/git-cache`. Falls back to ClusterIP service if discovery fails. Runner pods mount this path read-only.
...
| `base/kubernetes/git-cache/central-statefulset.yaml.tpl` | StatefulSet template (generated by `deploy.sh` with cluster config) |
| `base/kubernetes/git-cache/deploy.sh` | Generates StatefulSet + PDB from templates, applies to cluster |
