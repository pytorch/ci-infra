...
Defs live in `modules/arc-runners/defs/` (CPU runners + A100 GPU + release defs) and `modules/arc-runners-h100/defs/` (H100 GPU). The naming convention is identical across both modules — the split is operational (per-module capacity reservation handling), not semantic.
...
| `gpu_type` | No | GPU model (omitted for CPU-only runners) | `t4` = NVIDIA T4, `a10g` = NVIDIA A10G, `l4` = NVIDIA L4, `a100` = NVIDIA A100 40GB, `h100` = NVIDIA H100 |
...
| linux.arm64.m8g.metal-24xl | m8g.metal-24xl | mt-l-barm64g4-94-344 |
