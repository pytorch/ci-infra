...
- **H100 GPU userData (pre-DaemonSet)**: `modules/nodepools-h100/scripts/h100-node-setup.sh` reads from IMDS. EC2NodeClass userData runs BEFORE kubelet — needed because kubelet pulls the GPU device plugin image during startup and the DaemonSet hasn't started yet.
...
| `modules/nodepools-h100/scripts/h100-node-setup.sh` | GPU EC2NodeClass userData: writes /etc/hosts entry (IMDS lookup) and containerd mirror configs before kubelet starts (so the GPU device plugin pull succeeds) |
