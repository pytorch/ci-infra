...
- **Instance types**: Configurable via `clusters.yaml` (`buildkit.arm64_instance_types`, `buildkit.amd64_instance_types`) — a map of instance type to pods per node. A NodePool may span several sizes (amd64 runs m6id.24xlarge + m6id.12xlarge) so Karpenter has a fallback when one hits InsufficientInstanceCapacity.
