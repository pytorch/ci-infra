...
**A GPU is a capability, not a selector.** Like the clone repository and the model, it is
decided by `authorize.py` and reaches the Job through the `Grant` — `job_manifest()` never
sees the request body. `ALLOWED_CALLERS` carries a per-caller `gpu` flag, read
default-deny, so a caller entry that omits it cannot have one. It differs from repo and
model in being opt-in per request: an authorized caller asks, and most of its runs will
not. Asking without permission is a **403**, never a silent CPU run — a downgraded task
returns no `gpu` block, which is indistinguishable from nvproxy failing to inject the
device. An **unauthenticated** caller cannot get one at all while `REQUIRE_AUTH` is false:
the migration window must not be the way to skip the capability check.
