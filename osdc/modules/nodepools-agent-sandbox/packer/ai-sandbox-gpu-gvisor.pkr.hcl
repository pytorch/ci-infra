...
#      than `recommended`. nvproxy only supports an explicit list of driver versions, so
#      the release is chosen for the driver it ships and not for being newest. See
#      docs/agent-sandbox-gpu-gvisor.md for the pinning policy — driver and runsc move
#      together, and neither moves alone. The CPU AMI tracks `recommended` precisely
...
variable "gvisor_release" {
  ...
  #
  # 20260831 is the FLOOR for the driver pinned below — it is the release that added
  # 580.178.04 ("nvproxy: add support for driver 580.178.04"); 20260824 and everything
  # before it stop at 580.173.02. Do not lower this without also lowering
  # nvidia_ami_release, and read the kernel note there before doing that.
  default = "20260831"
}
...
variable "nvidia_ami_release" {
  ...
  type        = string
  # v20260903 ships driver 580.178.04 and kernel 6.12.103-127.188.amzn2023.
  #
  # The kernel is why this is not v20260810 any more: that release stops at
  # 6.12.95-124.187, which predates the KVM shadow-MMU use-after-free fix
  # (CVE-2026-64561 "Zapscape", first in v20260818). Every NVIDIA release carrying that
  # fix ships 580.178.04 — there is no patched-kernel-with-an-older-driver option — which
  # is what set the gVisor floor above.
  #
  # Both halves have to be checked together before moving this. The driver:
  #   gh api /repos/awslabs/amazon-eks-ami/releases/tags/<tag> --jq .body | grep -A1 kmod-nvidia
  # then confirm that version is listed by `runsc nvproxy list-supported-drivers` for the
  # pinned gvisor_release. install-gvisor.sh re-checks it at build time, so a mismatch
  # fails the build rather than the fleet — but it cannot tell you the kernel is too old,
  # which only this comment does.
  default = "v20260903"
}
