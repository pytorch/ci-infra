...
# Called by: just deploy (as the provider module)
# Args: $1=cluster-id  $2=cluster-name  $3=region
#
# Handles all AWS/EKS base infrastructure:
#   1. Terraform: VPC + EKS cluster + Harbor S3/IAM
#   2. Kubeconfig
#   3. Mirror Harbor bootstrap images to ECR
#   4. Apply base Kubernetes resources (EKS-specific)
#   5. Deploy Git Cache (central + DaemonSet)
#   6. Deploy Harbor (S3 storage backend)
#   7. Deploy Node Compactor
...
# ─── Phase 4: Base Kubernetes resources ───────────────────────────────────
echo ""
echo "━━━ EKS: Kubernetes resources ━━━"
kubectl apply -k "$MODULE_DIR/kubernetes/"

# ─── Phase 5: Git Cache ──────────────────────────────────────────────────
echo ""
echo "━━━ EKS: Deploy Git Cache Central ━━━"
"$MODULE_DIR/kubernetes/git-cache/deploy.sh" "$CLUSTER"
...
# ─── Phase 7: Node Compactor ─────────────────────────────────────────────
echo ""
echo "━━━ EKS: Deploy Node Compactor ━━━"
"$MODULE_DIR/node-compactor/deploy.sh" "$CLUSTER"
