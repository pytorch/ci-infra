...
set -euo pipefail
#
# Harbor pull-through cache deploy script.
# Called by: just deploy-module
# Args: $1=cluster-id  $2=cluster-name  $3=region
#
# Deploys Harbor as a pull-through cache with S3 storage backend.
# Reads terraform outputs from the EKS base module for IAM/S3 config.

CLUSTER="$1"
# $2=cluster-name, $3=region — unused by harbor but passed by deploy-module
...
# ─── Resolve EKS module (for terraform outputs) ────────────────────────
OSDC_ROOT="${OSDC_ROOT:-$UPSTREAM_ROOT}"
if [[ -d "$OSDC_ROOT/modules/eks" ]]; then
  EKS_DIR="$OSDC_ROOT/modules/eks"
else
  EKS_DIR="$UPSTREAM_ROOT/modules/eks"
fi
...
# ─── Get Terraform outputs from EKS state ───────────────────────────────
cd "$EKS_DIR/infra"
