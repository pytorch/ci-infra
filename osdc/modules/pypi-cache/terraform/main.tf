...
# --- Wheel Syncer IAM Role (IRSA) ---

resource "aws_iam_role" "wheel_syncer" {
  name = "${var.cluster_name}-pypi-wheel-syncer-role"
  ...
  assume_role_policy = jsonencode({
    ...
    Statement = [{
      ...
      Condition = {
        StringEquals = {
          "${local.oidc_provider}:aud" = "sts.amazonaws.com"
          "${local.oidc_provider}:sub" = "system:serviceaccount:pypi-cache:pypi-wheel-syncer"
        }
      }
    }]
  })
...
resource "aws_iam_policy" "wheel_syncer" {
  name = "${var.cluster_name}-pypi-wheel-syncer-s3"
...
resource "aws_iam_role_policy_attachment" "wheel_syncer" {
  policy_arn = aws_iam_policy.wheel_syncer.arn
  role       = aws_iam_role.wheel_syncer.name
}
