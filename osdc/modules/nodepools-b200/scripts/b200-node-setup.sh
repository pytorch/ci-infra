...
if [[ -z "$NODE_IPV6" ]]; then
  echo "ERROR: [b200-node-setup] Could not resolve node IPv6 from IMDS; failing fast so Karpenter retries" >&2
  exit 1
fi
if [[ ! "$NODE_IPV6" =~ ^[0-9a-fA-F:]+$ ]]; then
  echo "ERROR: [b200-node-setup] IMDS returned non-IPv6 value '$NODE_IPV6'; failing fast" >&2
  exit 1
fi
