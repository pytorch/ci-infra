...
NODEPOOLS_AZS=$(uv run "$CLUSTER_CONFIG" "$CLUSTER" azs)
NODEPOOLS_VALID_BUCKETS=$(uv run "$CLUSTER_CONFIG" "$CLUSTER" valid-buckets)
...
NODEPOOLS_DEFS_DIR="$DEFS_DIR" \
  ...
  NODEPOOLS_AZS="$NODEPOOLS_AZS" \
  NODEPOOLS_VALID_BUCKETS="$NODEPOOLS_VALID_BUCKETS" \
