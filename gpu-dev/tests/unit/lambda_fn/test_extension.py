...
def _record(extension_hours=24):
    return {
        "body": json.dumps(
            {
...
def test_single_gpu_extension_has_no_total_duration_limit(
    lambda_index, monkeypatch, aws_mocks
):
    reservation = _reservation(Decimal("1"))
    update_error = _patch_extension_dependencies(lambda_index, monkeypatch, reservation)

    assert lambda_index.process_extend_reservation_action(_record()) is True
...
def test_multi_gpu_extension_keeps_total_duration_limit(
    lambda_index, monkeypatch, aws_mocks
):
    reservation = _reservation(Decimal("8"))
    update_error = _patch_extension_dependencies(lambda_index, monkeypatch, reservation)
