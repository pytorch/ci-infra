...
# Markers left by third-party tools that copy our snapshots. Add to these when
# a new copier shows up; a copier absent from this list still reads as ours.
_SNAPSHOT_COPY_TAG_PREFIXES = ("wiz",)
_SNAPSHOT_COPY_DESCRIPTIONS = ("auto-generated by Wiz",)


def is_third_party_snapshot_copy(snapshot: dict[str, Any]) -> bool:
    """Whether a third-party tool produced this snapshot by copying one of ours.
    ...
    This detects known copiers rather than positively proving ownership. A
    `SnapshotType` allowlist would be stronger but is not available: 133 of
    ~6600 real snapshots carry no such tag -- `process_clone_disk_action`
    never sets it on live-clone snapshots, and there are hand-made recovery
    snapshots besides -- so requiring it would orphan real restore points.
    """
    tags = {tag["Key"]: tag["Value"] for tag in snapshot.get("Tags", [])}
    if any(key.lower().startswith(_SNAPSHOT_COPY_TAG_PREFIXES) for key in tags):
        return True
    # A copy has no source volume in this account.
    if snapshot.get("VolumeId") == "vol-ffffffff":
        return True
    description = snapshot.get("Description") or ""
    return any(marker in description for marker in _SNAPSHOT_COPY_DESCRIPTIONS)


def filter_own_snapshots(snapshots: list, context: str = "") -> list:
    """Drop third-party copies, naming what was excluded.

    Every other branch around the snapshot decision logs; without this an
    exclusion would leave a plausible-looking restore with nothing in
    CloudWatch explaining which snapshot was passed over, or why.
    """
    ours, copies = [], []
    for snapshot in snapshots:
        (copies if is_third_party_snapshot_copy(snapshot) else ours).append(snapshot)
    if copies:
        logger.info(
            f"Ignoring {len(copies)} third-party snapshot copy/copies"
            + (f" ({context})" if context else "")
            + ": " + ", ".join(s.get("SnapshotId", "?") for s in copies)
        )
    return ours
...
def process_clone_disk_action(record: dict[str, Any]) -> bool:
    ...
    try:
        ...
        # Exclude soft-deleted and third-party copies. This pick is persisted as
        # clone_source_snapshot, so a copy here is durable: once the scanner
        # deletes it, every reservation of the cloned disk fails and the disk
        # can never take a snapshot of its own to escape the reference.
        active_snapshots = filter_own_snapshots([
            s for s in snapshots
            if 'delete-date' not in {t['Key']: t['Value'] for t in s.get('Tags', [])}
        ], f"clone source for '{source_disk}'")
